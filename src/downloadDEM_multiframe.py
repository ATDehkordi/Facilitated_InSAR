...
class DEMdownloader_multiframe:

    def __init__(self, path_base, path_work):
        self.path_base = path_base
        self.path_work = path_work
        self.path_base1 = os.path.join(self.path_base, '1')
        self.path_base2 = os.path.join(self.path_base, '2')

    def get_first_safe_annotation_xml_files(self):

        # List all .SAFE directories in the parent directory
        safe_dirs1 = sorted([d for d in os.listdir(self.path_base1) if d.endswith('.SAFE')])
        safe_dirs2 = sorted([d for d in os.listdir(self.path_base2) if d.endswith('.SAFE')])

        if not safe_dirs1:
            print('No .SAFE file was found in folder 1')
        if not safe_dirs2:
            print('No .SAFE file was found in folder 1')

        # Get the first .SAFE directory (sorted alphabetically)
        first_safe_dir1 = safe_dirs1[0]
        first_safe_dir2 = safe_dirs2[0]
        # Path to the annotation folder within the first .SAFE directory
        annotation_folder1 = os.path.join(self.path_base1, first_safe_dir1, 'annotation')
        annotation_folder2 = os.path.join(self.path_base2, first_safe_dir2, 'annotation')
        
        # List all .xml files in the annotation folder
        xml_files1 = glob.glob(os.path.join(annotation_folder1, '*.xml'))
        xml_files2 = glob.glob(os.path.join(annotation_folder2, '*.xml'))
        return xml_files1, xml_files2
    ...
    def process(self):
        ...
        xml_file_paths1,  xml_file_paths2 = self.get_first_safe_annotation_xml_files()

        all_bounding_boxes = []
        for xml_file in xml_file_paths1:
            bounding_box1 = self.get_boundingbox_from_xml(xml_file)
            all_bounding_boxes.append(bounding_box1)

        for xml_file in xml_file_paths2:
            bounding_box2 = self.get_boundingbox_from_xml(xml_file)
            all_bounding_boxes.append(bounding_box2)
