...
foreach line (`awk 'NR>1{print $1}' data.in`)
  ...
  rm -f $dmaster"_"$daligned.csh
  echo "cd $dmaster"_"$daligned" >> $dmaster"_"$daligned.csh
  if ($mode == 1) then
    echo "preproc_batch_tops.csh $file $dem 1 >& log " >> $dmaster"_"$daligned.csh
  else
  ...
  echo "csh $dmaster'_'$daligned.csh > log_$dmaster'_'$daligned" >> preproc.cmd
end
